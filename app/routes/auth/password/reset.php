...
$app->post('/password-reset', $guest(), function() use ($app){
    ...
    $v->validate([
        ...
        'pasword_confirm|Confirm password' => [$passwordConfirm, 'required|matches(password)'],
    ...
    if ($v->passes()){
        ...
        $app->flash('global', 'Your password has been reset and you can sign in now.');
       return $app->response->redirect($app->urlFor('home'));
    }

    $app->render('auth/pasword/reset.php', [
        'errors' => $v->errors(),
        'email' => $user->email,
        'identifier' => $identifier
    ]);
